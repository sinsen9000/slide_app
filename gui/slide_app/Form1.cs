...
namespace slide_app
{
    public partial class Form1 : Form
    {
            ...
            for (int i = 1; i <= ppt_file.Slides.Count; i++)
            {
                ...
                foreach (string line in temp_lines)
                {
                    ...
                    foreach (var word in splitStr)
                    {
                        ...
                        if (new Regex(@"(。|？|\?|！|\!|（|「|\(|）|」|\))").IsMatch(word.ToString()))
                        {
                            ...
                            if (is_bracket)
                            {
                                ...
                                if (new Regex(@"(）|\))").IsMatch(word.ToString()))
                                {
                                    if (ModeCombo.Text == "normal") notes[notes.Count() - 1].Bracket = bracket_sentence[..^1];
                                    else
                                    {
                                        notes[notes.Count() - 1].Sentence += ("（" + bracket_sentence[..^1] + "）");
                                        notes[notes.Count() - 1].Pnt = "）";
                                    }
                                    bracket_sentence = "";
                                    is_bracket = false;
                                }
                            ...
                            {
                                // 音声文の生成
                                if (notes.Count > 0 && new Regex(@"「").IsMatch(notes[notes.Count - 1].Sentence) && new Regex(@"」").IsMatch(t)) notes[notes.Count - 1].Sentence = notes[notes.Count - 1].Sentence + t;
                                else if (ModeCombo.Text == "visual" && new Regex(@"(）|\))").IsMatch(notes[notes.Count() - 1].Pnt))
                                {
                                    notes[notes.Count() - 1].Sentence += t;
                                    notes[notes.Count() - 1].Pnt = words[words.Count - 1];
                                }
                                else
                                {
                                    if (new Regex(@"(（|「|\(|「)").IsMatch(t)) t = t[..^1];
                                    if (notes.Count > 0 && notes[notes.Count - 1].Pnt == "「") t = "「" + t;

                                    num += 1;
                                    int motion_ID = Motion_ID(words.Count);
                                    Cue_card m = new Cue_card { No = num, Num = i, Id = motion_ID, Pnt = words[words.Count - 1], Sentence = t, Bracket = "", Size = words.Count };
                                    notes.Add(m);
                                }

                            }
                            else if (ModeCombo.Text == "visual" && new Regex(@"(。|？|\?|！|\!)").IsMatch(t)) notes[notes.Count() - 1].Sentence += t;
                    ...
                    if (words.Count() > 0 && t != " ")
                    {
                        int motion_ID = Motion_ID(words.Count);
                        num = num + 1;
                        Cue_card m = new Cue_card { No = num, Num = i, Id = motion_ID, Pnt = "。", Sentence = String.Join("", words.ToArray()), Bracket = "", Size = words.Count };
                        notes.Add(m);
                    }
                }
            }
        ...
        private void GenerateButton_Click(object sender, EventArgs e)
        {
            ...
            Cue_card m_last_1 = new Cue_card { No = notes[notes.Count - 1].No + 1, Num = notes[notes.Count - 1].Num, Id = 0, Pnt = "。", Sentence = String.Format("この動画は、voicevox{0}の音声でお送り致しました。", VoiceNameCombo.Text), Bracket = String.Format("VOICEVOX: {0}", VoiceNameCombo.Text), Size = 10 };
            last_list.Add(m_last_1);
            notes.Add(m_last_1);
            Cue_card m_last_2 = new Cue_card { No = m_last_1.No + 1, Num = m_last_1.Num, Id = 21, Pnt = "。", Sentence = String.Format("ここまでのご視聴、ありがとうございました。", VoiceNameCombo.Text), Bracket = String.Format("VOICEVOX: {0}", VoiceNameCombo.Text), Size = 10 };
